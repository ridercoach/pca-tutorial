...
investigate how PCA can augment *supervised* learning; specifically, I want 
to examine how the performance of a simple model is affected by:

1. subsetting the original features based on insight gained from PCA;
1. replacing the original features with some small set of PCA "features".

# PCA and Supervised Learning

PCA finds the directions along which a cloud of data points varies the 
most.^[If our data includes *n* features, then each observation is a point in 
n-dimensional space, and all the observations together might be envisioned 
as a "cloud" of these points.]  It does not include any notion of one feature 
being the "target" or "output", and therefore is often considered a technique 
for *un*supervised learning.  It can also be used during EDA when we are simply 
trying to become familiar with the data.

However, because PCA usually enables us to convert our existing features to 
a much smaller set of "better" features, it can also be used as a preprocessor 
for supervised learning, and that is what we will explore in this article. Note 
that when using PCA for this purpose, we do not include the target feature in the PC 
analysis.^[If we did, we would "lose" the target feature, because it would be 
mixed into the principal components with all the other features. (This will make 
...
This data set is very small, which is good for our purposes here, 
but which prevents us from doing rigorous modeling with cross-validation, 
etc.  Still, we can fit a line and check the RSE, and I think that is sufficient.
...
data_base <- as.data.frame(cbind(data[,1], scaled_X))
colnames(data_base)[1] <- "mpg"
knitr::kable(
  format(data_base[1:4,], digits = 3), 
  caption = 'Data for creating the baseline model'
)
```

And here is the RSE of the baseline model:

```{r mod_base}
mod_base <- lm(mpg ~ ., data_base)
sigma(mod_base)
```
