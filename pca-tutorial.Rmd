...
Continental", because the model gives a prediction close to reality (small residual):

```{r linc_res}
mod_pca2$residuals["Lincoln Continental"]
```

Here is the centered and scaled data for this observation in the *original* 
(non-PCA) coordinate system:

```{r linc_orig_data}
knitr::kable(
  format(data_baseline["Lincoln Continental",], digits = 3), 
  caption = 'Lincoln Continental, centered and scaled'
)
```

Before we can use this data as input to the model, we must transform it to the
PCA coordinate system, which is done by matrix-vector multiplication.^[You can
learn about this from many online sources, including the
[3blue1brown](https://www.3blue1brown.com) website mentioned previously.] Recall 
that the information returned by `prcomp()` includes the matrix we need to 
do this:

```{r linc_xfrm_data}
# vector in original coordinate system, transposed
v_old <- t(data_baseline["Lincoln Continental",2:9])
# transformation matrix, transposed
A <- t(pca2$rotation)
# matrix-vector mult yields vector in PCA coordinate system
v_new <- A %*% v_old

knitr::kable(
  format(t(v_new), digits = 3), 
  caption = "Lincoln Continental, transformed"
)
```

If we compare this to the corresponding row in the transformed data returned 
by `prcomp()`, we can see that we have done the conversion correctly:

```{r linc_xfrm_comp}
df_tmp <- as.data.frame(pca2$x)
knitr::kable(
  format(df_tmp["Lincoln Continental",], digits = 3), 
  caption = 'Same data transformed by `pcomp()`'
)
```

And as the final step in checking our understanding, let's take the 
data that *we* transformed to PCA coordinates, and pass the first three 
components of it to the model to get a prediction:

```{r linc_predict}
predict(mod_pca2, data.frame("PC1" = v_new[1], 
               "PC2" = v_new[2], "PC3" = v_new[3]))
```

The above result matches the combination of the actual **mpg** for the test 
case and the residual returned from fitting the model, so we have verified that 
we know how to use the model to get a prediction.
...
#                  "gear" = 5, "carb" = 2)
data_wombat <- data.frame("cyl" = 8, "disp" = 460, "hp" = 215, 
                  "drat" = 3, "wt" = 5.424, "qsec" = 17.82, 
                  "gear" = 3, "carb" = 4)
rownames(data_wombat)[1] <- "Super Wombat"
knitr::kable(data_wombat,
  caption = 'Original feature values, Super Wombat'
)
```

Transform the data to PCA coordinates and feed the first three components 
to the model:

```{r new_pred}
v_old <- t(data_wombat[1,])
v_new <- A %*% v_old
predict(mod_pca2, data.frame("PC1" = v_new[1], 
               "PC2" = v_new[2], "PC3" = v_new[3]))
```
